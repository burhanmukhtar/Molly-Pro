// Enhanced email processing service with personalized attachments and custom MIME boundaries

const nodemailer = require('nodemailer');
const logger = require('./logger');
const config = require('./config');
const { processDynamicTags, generateCustomTagValues, Semaphore, generateRandomString } = require('./utils');
const { createPersonalizedAttachment, closeBrowser, getBrowser } = require('./attachment-service');
...
async function getTransporter(smtpInfo) {
  ...
  if (!transporterPool.has(key)) {
    ...
    const transporter = nodemailer.createTransport({
      ...
      maxConnections: 8,  // Maximum number of connections per SMTP server
      maxMessages: 200    // Maximum number of messages per connection
    });
...
async function processEmails(
  ...
  try {
    ...
    let concurrencyPerSmtp = Math.max(2, Math.floor(threadCount / smtpCount));
    ...
    // Initialize the browser if we're sending attachments
    if (attachmentType !== 'none') {
      logger.info('Initializing browser for attachment generation');
      await getBrowser();
    }
    ...
    // Close browser instance if it was opened
    try {
      await closeBrowser();
    } catch (error) {
