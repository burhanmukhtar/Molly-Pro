...
GOOGLE_APPLICATION_CREDENTIALS = os.environ.get('GOOGLE_APPLICATION_CREDENTIALS', './molly-pro-452801-5d2f409d651c.json')
gcp_project_id = os.environ.get('GCP_PROJECT_ID', 'molly-pro-452801')
gcp_zone = os.environ.get('GCP_ZONE', 'europe-west1-b')  # Default zone
gcp_region = os.environ.get('GCP_REGION', 'europe-west1')  # Default region

# Define available regions and zones (all US regions for better image compatibility)
# Define available regions and zones (EU regions only)
AVAILABLE_REGIONS = [
    # EU Regions
    {
        'region': 'europe-west1',
        'zones': ['europe-west1-b', 'europe-west1-c', 'europe-west1-d'],
        'description': 'Belgium, EU West'
    },
    {
        'region': 'europe-west2',
    ...
    },
    {
        ...
        'description': 'Netherlands, EU West'
    },
    {
        'region': 'europe-west6',
...
# Maximum number of instances allowed per region
...
# Function to get servers count by region
def get_server_counts_by_region():
    """
    Get the current count of active servers in each region
    Returns a dictionary with region as key and count as value
    """
    try:
        # Find all active servers (not expired)
        ...
        for region in AVAILABLE_REGIONS:
            region_counts[region['region']] = 0
            
        counts = list(db.Servers.aggregate(pipeline))
        for count_doc in counts:
            if count_doc['_id'] in region_counts:
                region_counts[count_doc['_id']] = count_doc['count']
                
        return region_counts
    except Exception as e:
        logger.error(f"Error getting server counts by region: {str(e)}")
        # Return all zeros on error
        return {region['region']: 0 for region in AVAILABLE_REGIONS}

# Function to get the best region and zone for a new server
def get_available_region_and_zone():
    """
    Select the best region and zone for a new server
    Tries to balance servers across regions
    Returns (region, zone) tuple or None if all regions are at capacity
    """
    try:
        # Get current counts by region
        region_counts = get_server_counts_by_region()
        
        # Find regions below capacity
        available_regions = [
            region for region in AVAILABLE_REGIONS
            if region_counts[region['region']] < MAX_INSTANCES_PER_REGION
        ]
        
        if not available_regions:
            logger.warning("All regions are at capacity")
            return None, None
            
        # Select region with fewest instances
        selected_region = min(available_regions, key=lambda r: region_counts[r['region']])
        
        # Randomly select a zone within the region
        selected_zone = random.choice(selected_region['zones'])
        
        logger.info(f"Selected region {selected_region['region']} (zone {selected_zone}) with {region_counts[selected_region['region']]} instances")
        
        return selected_region['region'], selected_zone
    except Exception as e:
        logger.error(f"Error selecting region and zone: {str(e)}")
        # Fallback to default region/zone
        default_region = AVAILABLE_REGIONS[0]['region']
        default_zone = AVAILABLE_REGIONS[0]['zones'][0]
        return default_region, default_zone

# Migration function to set up database indexes and update existing records
def migrate_database():
    """Migrate the database to support multiple regions"""
    try:
        # 1. Create index on gcp_region for faster querying
        logger.info("Creating indexes on Servers collection")
        db.Servers.create_index("gcp_region")
        db.Servers.create_index([("user_id", 1), ("expiresAt", 1)])
        
        # 2. Update any existing server records to have proper region/zone
        current_time = datetime.datetime.now(datetime.UTC)
        ...
        if not servers:
            logger.info("No active servers found to migrate")
            return
            
        # For each active server, ensure it has proper region/zone
        logger.info(f"Found {len(servers)} active servers to migrate")
        
        updates = []
        for server in servers:
            # Skip if already has both fields
            if 'gcp_region' in server and 'gcp_zone' in server:
                continue
                
            # Set default region/zone if missing
            if 'gcp_zone' not in server:
                server['gcp_zone'] = 'us-central1-c'
                
            if 'gcp_region' not in server:
                # Extract region from zone if possible
                if 'gcp_zone' in server and '-' in server['gcp_zone']:
                    parts = server['gcp_zone'].split('-')
                    if len(parts) >= 2:
                        server['gcp_region'] = f"{parts[0]}-{parts[1]}"
                    else:
                        server['gcp_region'] = 'us-central1'
                else:
                    server['gcp_region'] = 'us-central1'
            ...
            updates.append(
                UpdateOne(
                    {'_id': server['_id']},
                    {'$set': {
                        'gcp_region': server['gcp_region'],
                        'gcp_zone': server['gcp_zone']
                    }}
                )
            )
        ...
        if updates:
            result = db.Servers.bulk_write(updates)
            logger.info(f"Updated {result.modified_count} server records with region/zone information")
...
# Helper function for IP rotation background task
    ...
    try:
        ...
        for rule_config in required_rules:
            if rule_config['name'] not in existing_rules:
                ...
                # Set target tags if specified
                if 'target_tags' in rule_config:
...
# Function to terminate expired servers
def terminate_expired_servers():
    try:
        ...
        for server in expired_servers:
            try:
                # Get the server's region and zone
                zone = server.get('gcp_zone', gcp_zone)  # Default to global zone if not found
                region = server.get('gcp_region', gcp_region)  # Default to global region if not found
                ...
                if server.get('type') == 'unlimited':
                    try:
                        # Don't use filter parameter, manually filter instead
                        static_ips = []
                        for address in addresses_client.list(
                            project=gcp_project_id,
                            region=region
                ...
                try:
                    ...
                    if instance_info.status not in ['TERMINATED', 'STOPPING']:
                        ...
                        wait_for_operation(operation, zone=zone)
                    else:
                ...
                logger.info(f"Terminated expired server: {server['instanceId']} in {region}/{zone}")
            except Exception as e:
...
# Continuation of the regions endpoint
...
def get_regions(current_user):
    try:
        ...
        # Format regions with counts and availability
        regions_data = []
        for region_info in AVAILABLE_REGIONS:
            region = region_info['region']
            count = region_counts.get(region, 0)
            available = count < MAX_INSTANCES_PER_REGION
            
            regions_data.append({
                'region': region,
                'description': region_info['description'],
                'currentInstances': count,
                'maxInstances': MAX_INSTANCES_PER_REGION,
                'available': available
            })
        ...
        return jsonify({'message': f'Failed to retrieve regions: {str(e)}', 'regions': []}), 500
...
def terminate_server(current_user, server_id):
    try:
        ...
        try:
            ...
            if server['instanceId'].startswith('molly-server-'):
                # Get zone and region from server record
                zone = server.get('gcp_zone', gcp_zone)
...
# The main server creation function - updated for multi-region support
...
def create_server(current_user):
    ...
    # Get available region and zone using the selection algorithm
    region, zone = get_available_region_and_zone()
    
    if not region or not zone:
        return jsonify({'message': 'No regions available for new servers. Please try again later.'}), 503
    ...
    try:
        ...
        logger.info(f"Creating GCP instance for user {current_user['username']} in {region}/{zone} using image {custom_image_name}")
...
cd /home/black19990521/home/black19990521/backend
nohup node /home/black19990521/backend/server.js > node-server.log 2>&1 &
"""
        ...
        instance.machine_type = f"zones/{zone}/machineTypes/e2-standard-2"  # Equivalent to t2.medium
        instance.disks = [disk]
        ...
        instance.labels = {
            ...
            "service": "mollyserver",
            "region": region.replace('-', '_')  # GCP labels can't have hyphens
        }
        
        # Add network tags for firewall rules
        instance.tags = compute_v1.Tags()
        instance.tags.items = ["mollyserver"]
        ...
        logger.info(f"GCP instance {vm_name} is running in {region}/{zone} with IP {public_ip}")
...
def check_server_status(current_user, server_id):
    try:
        ...
        server_data = {
            ...
            'gcp_region': server.get('gcp_region', gcp_region),
            'gcp_zone': zone
        }
...
if __name__ == '__main__':
    # Run database migration to set up indexes and update records for multi-region support
    try:
        migrate_database()
        logger.info("Database migration completed successfully")
    except Exception as e:
